  ...
  self.update = function(after) {
    ...
    $.get(self.remoteUrl, function(data) {
      ...
      for (var z in namevaluepairs) {
        ...
        var units = "";
        if (namevalue[0].indexOf("T1") === 0) units = String.fromCharCode(176) + "C"; /*setting up the units to display: Temperature*/
        if (namevalue[0].indexOf("H") === 0) units = String.fromCharCode(37); /*Humidity*/
...
function MainViewModel() {
  ...
  //define logs behaviour depending on whether the log-check boxes are enabled or not
  ...
  //define latest_data_section behaviour depending on whether the log-check boxes are enabled or not
  ...
  //log the individual the caused the change
    ...
    self.config.update(function() {
      self.status.update(function() {
        self.last.update(function() {
          self.init(function() {
            self.connect();
            //set the logs and the latest data modes from the js_cookies
  ...
  //---------------------------------------------------------------------
  // Initialize graph and relay slider switches
  //---------------------------------------------------------------------
  ...
  self.update = function() {
    ...
    self.status.update(function() {
      //after status update, refresh relay states without reloading the page by invocation below
  ...
  self.refreshRelayStates = function() {
    //take array of available relays set the state as the state received
  ...
  self.onSwitchClick = function(item) {
    ...
    self.sendRelayMsg(item, item.state()); //Send message on the state of button on front end to ESP8266 server
    return true;
  };
  ...
  self.socket = false;
  self.wsEndpoint = "ws://" + baseHost + '/ws'; //shouldn't we have double quotes here?
  ...
  //-------------------------------------------------------------------------
  // Websocket's Relay Message function definition
  //-------------------------------------------------------------------------
  self.sendRelayMsg = function(relay, state) {
    //message construction of what is to be sent to ESP8266 Server
    var msg = {
    ...
    //Next we convert the above message to JSON format aand log
    console.log(JSON.stringify(msg));
    //Send the JSON converted data
    self.socket.send(JSON.stringify(msg));
  };

  //-------------------------------------------------------------------------
  // Cookie manager code: referred https://www.w3schools.com/js/js_cookies.asp
  //-------------------------------------------------------------------------
